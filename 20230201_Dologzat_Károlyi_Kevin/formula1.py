...
def sebesseg(idokor, lengthOfCircuit):
    ...
    masodperc = (int(splitted[0]) * 60) + (int(splitted[2]) / 60 + int(splitted[1]))
    atlagSebesseg = (lengthOfCircuit / masodperc) * 3.6
...
for i in range(3):
    ...
    print(f'{nev} versenyző átlagsebessége: {round(sebesseg(idokor,length(circuit)), 2)}')